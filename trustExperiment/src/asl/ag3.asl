...
+!initialImps : true
<-  !evaluateProvider(ag1, "CARRY", ["v1","v2","v3"], [0.2,0.5,0.8]);
	!evaluateProvider(ag1, "CARRY", ["v1","v2","v3"], [0.3,0.6,0.8]);
	!evaluateProvider(ag1, "CARRY", ["v1","v2","v3"], [0.4,0.7,0.9]);
	!computeImage(ag1, "CARRY", ["v1","v2","v3"]);
	!sendImage(ag1, ag2, "CARRY");
	!test.

+!test: true
<-	!evaluateProvider(ag1, "CARRY", ["v1","v2","v3"], [0.2,0.5,0.7]);	
	!computeImage(ag1, "CARRY", ["v1","v2","v3"]);	
	!sendImage(ag1, ag2, "CARRY");
	
	!evaluateProvider(ag2, "CARRY", ["v1","v2","v3"], [0.2,0.5,0.9]);
	!computeImage(ag2, "CARRY", ["v1","v2","v3"]);	
	!sendImage(ag2, ag2, "CARRY");
	
	!evaluateProvider(ag2, "CARRY", ["v1","v2","v3"], [0.2,0.6,0.8]);
	!computeImage(ag2, "CARRY", ["v1","v2","v3"]);	
	!sendImage(ag2, ag2, "CARRY").
